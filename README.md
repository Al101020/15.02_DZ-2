Проблема вашего кода в том, что вы неправильно передаёте обработчик событий:

javascript
  return <button onClick={this.approveExpense(d.id)}>Approve</button>;
Используя непосредственно this.approveExpense(d.id) в коде JSX, вы сообщаете JavaScript, что нужно выполнить эту функцию, как только интерпретатор её прочитает. Вместо этого следует проксировать выполнение функции по клику, например так:

javascript
  return <button onClick={(e) => {this.approveExpense(d.id)}}>Approve</button>;
Более подробное объяснение того, как передавать функции компонентам в React, можно найти по ссылке https://reactjs.org/docs/faq-functions.html
===========================================================
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
===========================================================
ДЗ-2
...
![cards view](./public/assets/card_view.png)
...
![list_view](./public/assets/list_view.png)
...
git status
  Красным выделенно всё что не добавлено, зелёным то что добавлено.
  Теперь добавляем
git add -A
...
npm create vite@latest my-vue-app -- --template react
    перенести содержимое каталога в корневую папку:
mv my-vue-app/* ./
